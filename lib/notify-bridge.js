...
var NotifyBridge = function() {
  ...
  if (program.start) {
    ...
    console.log();
    console.log(' ############## notify-bridge started... ##############');
    console.log();

    console.log(' On the Serverside you can now start sending JSON-RPC notifications with');
    console.log(' POST requests to this HTTP server which is listening on Port %s', program.httpPort);

    console.log();
    console.log(' On the Clientside (Browser) you need to embed Socket.IO like this: ');
    console.log(' <script src="http://<your-host>:%s/socket.io/socket.io.js"></script>', program.socketioPort);
    console.log();
    console.log(' ######################################################');
    console.log();
  } else {
